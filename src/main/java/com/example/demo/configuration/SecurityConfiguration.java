...
public class SecurityConfiguration {
	...
	SecurityFilterChain securityFilter(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests((req) -> req.requestMatchers(HttpMethod.DELETE, "/user/{id}", "/room/{id}")
				.hasRole("ADMIN").requestMatchers(HttpMethod.GET, "/users", "/bookings").hasRole(ADMIN)
				.requestMatchers(HttpMethod.PUT, "/room").hasRole(ADMIN).requestMatchers(HttpMethod.POST, "/room")
				.hasRole(ADMIN)

				.requestMatchers(HttpMethod.PUT, "/user").hasAnyRole(ADMIN, USER)
				.requestMatchers(HttpMethod.GET, "/availablerooms", "/rooms", "/room/{id}", "/booking/{id}")
				.hasAnyRole(ADMIN, USER).requestMatchers(HttpMethod.DELETE, "/booking/{id}").hasAnyRole(ADMIN, USER)
